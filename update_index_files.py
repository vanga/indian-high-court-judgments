...
class S3IndexInitializer:
    ...
    def __init__(self, bucket_name: str, year: str = None):
        ...
        print(f"Initialized S3IndexInitializer for bucket: {bucket_name}, year: {self.year}")
        print(f"Loaded {len(self.court_codes)} courts and {len(self.bench_codes)} benches")
    ...
    def _create_data_index(self, court_code: str, bench_name: str, updated_at: datetime) -> Dict:
        ...
        pdf_files = [os.path.basename(obj['Key']) for obj in pdf_objects if obj['Key'].endswith('.pdf')]
        ...
        tar_key = f"data/tar/year={self.year}/court={court_code}/bench={bench_name}/pdfs.tar"
    ...
    def _create_metadata_index(self, court_code: str, bench_name: str, updated_at: datetime) -> Dict:
        """Create metadata.index.json for a court/bench combination"""
        json_prefix = f"metadata/json/year={self.year}/court={court_code}/bench={bench_name}/"
        json_objects = self._list_s3_objects(json_prefix)
        json_files = [os.path.basename(obj['Key']) for obj in json_objects if obj['Key'].endswith('.json')]
                    ...
                    if 'CommonPrefixes' in page:
                        for common_prefix in page['CommonPrefixes']:
                            ...
                            if court_match:
                                ...
                                bench_paginator = self.s3_unsigned.get_paginator('list_objects_v2')
    ...
    def _find_latest_dates_for_courts(self, existing_structure: Dict[str, Set[str]]) -> Dict[str, datetime]:
    ...
    def _get_all_court_bench_combinations(self, existing_structure: Dict[str, Set[str]]) -> List[Tuple[str, str]]:
        ...
        for court_code_config in self.court_codes.keys():
            ...
            court_benches = [bench for bench, court in self.bench_codes.items() 
    ...
    def _create_empty_index(self, updated_at: datetime) -> Dict:
    ...
    def _upload_index_file(self, content: Dict, key: str) -> bool:
    ...
    def generate_all_indexes(self) -> bool:
        ...
        with tqdm(total=len(all_combinations), desc="Generating indexes", unit="combination") as pbar:
            for court_code, bench_name in sorted(all_combinations):
                try:
                    ...
                    has_existing_data = (court_code in existing_structure and 
                                       bench_name in existing_structure[court_code])
                    # Create data index
                    if has_existing_data:
                        data_index = self._create_data_index(court_code, bench_name, updated_at)
                    else:
                    ...
                    if has_existing_data:
                        metadata_index = self._create_metadata_index(court_code, bench_name, updated_at)
                    else:
                    ...
                    metadata_success = self._upload_index_file(metadata_index, metadata_key)
